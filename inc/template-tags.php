...
function wpsp_posted_on() {
	...
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">' . __( ' Edited %4$s', 'newstore' ) . '</time>';
	}
...
function wpsp_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		$categories_list = get_the_category_list( __( ', ', 'newstore' ) );
		if ( $categories_list && wpsp_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'newstore' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'newstore' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'newstore' ) . '</span>', $tags_list );
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( __( 'Leave a comment', 'newstore' ), __( '1 Comment', 'newstore' ), __( '% Comments', 'newstore' ) );
		echo '</span>';
	}
